...
export const agravoAiService={
 async generate(field:AgravoAiField,context:Record<string,unknown>,sourcePdf:File):Promise<string>{
   ...
   const response=await fetch('/api/ai/agravo-field',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${session.access_token}`},body:JSON.stringify({
     field,context,
